<script lang="ts">
  ...
  import { darkMode, toggleDarkMode } from "$lib/stores/themeStore";
  ...
  let tooltipTargetTheme: HTMLElement | null = null;
...
>
  ...
  <button
    ...
    class="flex items-center px-2 py-1 sm:px-4 sm:py-2 bg-surface-card text-icon border border-border rounded-md text-base font-medium transition-all duration-300 hover:bg-surface-card-hover hover:text-themed-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent whitespace-nowrap"
  >
    <svg
      class="mr-1 sm:mr-1"
      width="20px"
  ...
  {#if $showProgress}
    ...
    <button
      ...
      class="inline-flex items-center px-2 py-1 sm:px-4 sm:py-2 bg-surface-card text-icon border border-border rounded-md text-base font-medium transition-all duration-300 hover:bg-surface-card-hover hover:text-themed-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent whitespace-nowrap ml-1 sm:ml-2"
    >
      <svg
        class="mr-1"
        width="20px"
    ...
    <button
      ...
      aria-label="Scroll to current quest"
      ...
      {lastCheckedQuestId === null
        ? 'bg-surface-disabled text-themed-faint cursor-not-allowed'
        : 'bg-surface-card text-icon border border-border hover:bg-surface-card-hover hover:text-themed-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent'}"
  ...
  {#if $showScrollToTop}
    <button
      ...
      class="inline-flex items-center px-2 py-1 sm:px-2 sm:py-1 bg-surface-card text-icon border border-border rounded-md text-base font-medium transition-all duration-300 hover:bg-surface-card-hover hover:text-themed-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent whitespace-nowrap ml-1 sm:ml-2 mobile-flash sm:animate-pulse"
    >
      <svg
        width="20px"
        height="20px"
  ...
  <!-- Theme Toggle Button (desktop only, mobile follows system) -->
  <button
    bind:this={tooltipTargetTheme}
    on:click={toggleDarkMode}
    aria-label={$darkMode ? "Switch to light mode" : "Switch to dark mode"}
    class="hidden sm:inline-flex items-center px-2 py-1 sm:px-2 sm:py-1 bg-surface-card text-icon border border-border rounded-md text-base font-medium transition-all duration-300 hover:bg-surface-card-hover hover:text-themed-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent whitespace-nowrap ml-1 sm:ml-2"
  >
    {#if $darkMode}
      <svg
        width="20px"
    ...
    {:else}
      ...
      >
        <path
          d="M3.32031 11.6835C3.32031 16.6541 7.34975 20.6835 12.3203 20.6835C16.1075 20.6835 19.3483 18.3443 20.6768 15.032C19.6402 15.4486 18.5059 15.6834 17.3203 15.6834C12.3498 15.6834 8.32031 11.6541 8.32031 6.68359C8.32031 5.50585 8.55165 4.37884 8.96211 3.34766C5.65605 4.67975 3.32031 7.91648 3.32031 11.6835Z"
          stroke="currentColor"
          stroke-width="2"
          stroke-linecap="round"
          stroke-linejoin="round"
  ...
  <Tooltip
    targetElement={tooltipTargetTheme}
  ...
  <div bind:this={tooltipTargetToggle} class="flex items-center ml-1 sm:ml-2">
    <button
      ...
      aria-label="Auto-check quests"
      class={`w-10 h-5 sm:w-12 sm:h-6 flex items-center rounded-full p-1 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent ${
        $autoMode ? "bg-surface-disabled" : "bg-accent"
      }`}
    >
      <div
