...
RAW_QUESTS_CSV = (
    "https://raw.githubusercontent.com/xivapi/ffxiv-datamining/master/csv/Quest.csv"
)
RAW_EXVERSION_CSV = (
    "https://raw.githubusercontent.com/xivapi/ffxiv-datamining/master/csv/ExVersion.csv"
)
RAW_INSTANCE_CONTENT_CSV = "https://raw.githubusercontent.com/xivapi/ffxiv-datamining/master/csv/InstanceContent.csv"
...
XIV_BETA_API_INSTANCE_CONTENT_BASE_URL = (
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if response.status_code == 404:
                logging.info(
                    f"Resource not found: {url} with params {params}.")
                return None
            logging.warning(
...
def fetch_first_journal_entry(
    ...
    for folder_index in range(start_index, max_folder_number + 1):
        csv_url = (
            f"{RAW_JOURNAL_BASE_CSV_URL}/{str(folder_index).zfill(3)}/{quest_id}.csv"
        )
...
def fetch_instance_content(instance_id):
    ...
    if response is None:
        logging.warning(
            f"Failed to fetch instance content for ID {instance_id}.")
        return None
    ...
    try:
        ...
        return {
            ...
            "Image":
            fields.get("Image", {}).get("path_hr1", ""),
            "ContentTypeName":
            fields.get("ContentType", {}).get("fields",
                                              {}).get("Name", "Unknown"),
        }
    except Exception as e:
        logging.warning(
            f"Failed to parse instance content for ID {instance_id}: {e}")
    return None
...
def fetch_image_path(quest_name):
    ...
    try:
        ...
        logging.warning(
            f"Failed to parse image data for quest '{quest_name}': {e}")
    return None


def load_instance_content_mapping():
    response = requests.get(RAW_INSTANCE_CONTENT_CSV)
    if response.status_code == 200:
        csv_content = response.content.decode("utf-8")
...
def get_previous_quests(row):
    return [
        int(row[f"PreviousQuest[{i}]"]) for i in range(4)
...
def order_quests_by_next_msq(quests, group=None):
    ...
    start_quests = {quest["#"]
                    for quest in quests} - {
                        quest["NextMSQ"]
                        for quest in quests if quest["NextMSQ"]
                    }
    ...
    for start_quest_id in start_quests:
        ...
        while current_quest_id:
            ...
            if current_quest_id in processed_quests:
                logging.info(
                    f"Skipping duplicate quest ID {current_quest_id}.")
                break

            current_quest = quest_map[current_quest_id]
            sorted_quests.append(current_quest)
            processed_quests.add(
                current_quest_id)  # Mark this quest as processed
...
def resolve_unlocks_by_row(row):
    unlocks = []
...
if response.status_code == 200:
    ...
    logging.fatal(
        f"Failed to download Quest.csv. Status code: {response.status_code}")
    exit()

# Load the CSV content and include the first 1523 columns (including the penultimate column)
quest_data = pd.read_csv(StringIO(csv_content),
                         skiprows=[0, 2],
                         low_memory=False)
...
fetch_images = (
    True if auto_yes else
    input("Do you want to fetch images? (yes/no): ").strip().lower() == "yes")

# Prompt user if they want to fetch journal entries
fetch_journal_entries = (
    True if auto_yes else
    input("Do you want to fetch journal entries? (yes/no): ").strip().lower()
    == "yes")

# Prompt user if they want to fetch unlocks
fetch_unlocks = (
    True if auto_yes else
    input("Do you want to fetch unlocks? (yes/no): ").strip().lower() == "yes")

# # -> Quest (formatted) mapping
quests_by_number = {}
with tqdm(total=len(filtered_data), desc="Processing Quests",
          ncols=100) as pbar:
    for _, row in filtered_data.iterrows():
        ...
        quest_group = None
        expansion_name = get_expansion_name(row["Expansion"],
                                            expansion_mapping)
        ...
        if fetch_unlocks:
            ...
            unlocks = resolve_unlocks_by_row(row)
...
for _, envoy_quest in envoy_quests.iterrows():
    ...
    while current_quest is not None:
        ...
        for prev_id in previous_quest_ids:
            potential_quest = filtered_data.loc[
                (filtered_data["#"] == prev_id)
                & (filtered_data["EventIconType"] == 3)]
...
logging.info(
    f"After filtering no NextMSQ, {len(quests_by_number)} quests remain.")

# Organize the data into the desired structure with correct MSQ order
quests_by_expansion = {}
for quest in quests_by_number.values():
    expansion = quest["ExpansionName"]
    quest_group = (quest["QuestGroup"]
                   if quest["QuestGroup"] else QUEST_GROUP_MAIN_QUEST_LINE)
...
arr_quests = [
    quest for quest in quests_by_number.values()
    if quest["ExpansionName"] == expansion_mapping[0]
]
...
def validate_quest_order(sorted_quests, quests_by_number):
    for i in range(len(sorted_quests) - 1):
        ...
        if current_quest["NextMSQ"] != next_quest["#"]:
            expected_next = quests_by_number.get(current_quest["NextMSQ"],
                                                 {}).get("Name", "Unknown")
            logging.warning(
...
for group in QUEST_GROUPS:
    group_quests = [
        quest
        for quest in quests_by_expansion[expansion_mapping[0]].get(group, [])
    ]
    if group_quests:  # Only process if there are quests for this group
        sorted_quests_by_group[group] = order_quests_by_next_msq(
            group_quests, group)
...
len_limsa_lominsa = len(
    sorted_quests_by_group.get(QUEST_GROUP_LIMSA_LOMINSA, []))
logging.info(
...
main_quest_line_quests = [
    quest for quest in quests_by_expansion[expansion_mapping[0]].get(
        QUEST_GROUP_MAIN_QUEST_LINE, [])
]
if main_quest_line_quests:
    sorted_quests_by_group[
        QUEST_GROUP_MAIN_QUEST_LINE] = order_quests_by_next_msq(
            main_quest_line_quests, QUEST_GROUP_MAIN_QUEST_LINE)
    validate_quest_order(sorted_quests_by_group[QUEST_GROUP_MAIN_QUEST_LINE],
                         quests_by_number)
...
def validate_linked_list(starting_quests, quests_by_number):
    for start_quest_id in starting_quests:
        ...
        if not current_quest:
            logging.info(
                f"=> Starting quest with ID '{start_quest_id}' not found.")
            continue
